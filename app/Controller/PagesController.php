...
class PagesController extends AppController {
    ...
    public function admin_careers(){
    	...
    	$module_title = __('Karir');
    	$this->loadModel('Career');
		
		$options =  $this->Career->_callRefineParams($this->params, array(
			'limit' => Configure::read('__Site.config_new_table_pagination'),
		));
		$this->RmCommon->_callRefineParams($this->params);

        $this->paginate = $this->Career->getData('paginate', $options);
		$values = $this->paginate('Career');
	...
	public function admin_add_career() {

    	$module_title = __('Tambah Karir');
    	$urlRedirect = array(
            'controller' => 'pages',
            'action' => 'careers',
            'admin' => true
		...
		if( !empty($user) ) {
			...
			$data = $this->request->data;
			$result = $this->Career->doSave( $data );
	...
	public function admin_edit_career( $career_id ) {
        
        $module_title = __('Edit Karir');
        $urlRedirect = array(
            'controller' => 'pages',
            'action' => 'careers',
            'admin' => true
        ...
        $this->loadModel('Career');
        $career = $this->Career->getData('first', array(
        	'conditions' => array(
				'Career.id' => $career_id,
			),
		));
		...
		if( !empty($career) ) {
			...
			$data = $this->request->data;
			$result = $this->Career->doSave( $data, $career, $career_id );
			$this->RmCommon->setProcessParams($result, $urlRedirect);
			$this->RmCommon->_layout_file(array(
				'ckeditor',
			));
			...
			$this->RmCommon->redirectReferer(__('Karir tidak ditemukan'));
		}
    }

    public function admin_delete_multiple_career() {
		$data = $this->request->data;
		$id = $this->RmCommon->filterEmptyField($data, 'Career', 'id');

		$this->loadModel('Career');
    	$result = $this->Career->doDelete( $id );
		$this->RmCommon->setProcessParams($result, false, array(
			'redirectError' => true,
		));
    }
	...
	function career () {
		...
		$title_for_layout 		= sprintf('%s %s', $module_title, $url_without_http);
		$description_for_layout = sprintf(__('Halaman karir untuk info lowongan kerja di media online %s properti terbesar di indonesia. Anda dapat memasang iklan jual rumah, sewa rumah, dijual rumah atau mencari iklan rumah dijual, rumah disewakan, tanah dijual dengan mudah'), $url_without_http);
		...
		$og_meta = array(
			'title' => __('Peluang Karir di %s', Configure::read('Config.Company.data.UserCompany.name')),
			'image' => Configure::read('Config.Company.data.UserCompany.logo'),
			'path' => Configure::read('__Site.logo_photo_folder'),
			'size' => 'xm',
			'description' => __('%s membuka peluang kepada Anda untuk dapat bergabung bersama Kami.', Configure::read('Config.Company.data.UserCompany.name')),
		);

		$this->set(compact(
			'values', 'title_for_layout', 'description_for_layout', 'keywords_for_layout',
			'module_title', 'populers', 'agents', 'og_meta'
		));
	}
