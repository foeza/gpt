...
class RmUserComponent extends Component {
	...
	*/
	...
	function callRegisterIntegratedBeforeSave ( $user, $req_data ) {
		$this->controller->loadModel('UserIntegratedOrderAddon');
		
		if( !empty($user) ) {
			$_config = Configure::read('Config.Company.data');
			
			$user_id = Common::hashEmptyField($user, 'User.id', null);
			$name = Common::hashEmptyField($user, 'User.full_name');
			$email_agent = Common::hashEmptyField($user, 'User.email');
			$phone = Common::hashEmptyField($user, 'UserProfile.phone');
			$country_id = Common::hashEmptyField($user, 'UserProfile.country_id', null);
			$region_id = Common::hashEmptyField($user, 'UserProfile.region_id', null);
			$city_id = Common::hashEmptyField($user, 'UserProfile.city_id', null);
			$subarea_id = Common::hashEmptyField($user, 'UserProfile.subarea_id', null);
			$address = Common::hashEmptyField($user, 'UserProfile.address');
			$zip = Common::hashEmptyField($user, 'UserProfile.zip');

			$company_name = Common::hashEmptyField($_config, 'UserCompany.name');

			//set default from user profile
			$data['UserIntegratedOrder']['user_id'] = $user_id;
			$data['UserIntegratedOrder']['name_applicant'] = $name;
			$data['UserIntegratedOrder']['company_name'] = $company_name;
			$data['UserIntegratedOrder']['phone'] = $phone;
			$data['UserIntegratedOrder']['country_id'] = $country_id;
			$data['UserIntegratedOrder']['region_id'] = $region_id;
			$data['UserIntegratedOrder']['city_id'] = $city_id;
			$data['UserIntegratedOrder']['subarea_id'] = $subarea_id;
			$data['UserIntegratedOrder']['address'] = $address;
			$data['UserIntegratedOrder']['zip'] = $zip;
			$data['UserIntegratedOrder']['email_r123'] = $email_agent;
			$data['UserIntegratedOrder']['email_olx'] = $email_agent;
			$data['UserIntegratedOrder']['email_all_addon'] = $email_agent;

			// if request data
			if (!empty($req_data)) {
				$addon_olx = isset($req_data['UserIntegratedOrder']['addon_olx']) ? $req_data['UserIntegratedOrder']['addon_olx'] : 0;
				$addon_r123 = isset($req_data['UserIntegratedOrder']['addon_r123']) ? $req_data['UserIntegratedOrder']['addon_r123'] : 0;

				$r123_package_id = isset($req_data['UserIntegratedOrderAddon']['r123_package_id']) ? $req_data['UserIntegratedOrderAddon']['r123_package_id'] : 0;
				$olx_package_id = isset($req_data['UserIntegratedOrderAddon']['olx_package_id']) ? $req_data['UserIntegratedOrderAddon']['olx_package_id'] : 0;
				$is_all_addon = isset($req_data['UserIntegratedOrder']['is_email_all_addon']) ? $req_data['UserIntegratedOrder']['is_email_all_addon'] : 0;

				$subareas = !empty($subareas)?$subareas:0;
				$data['UserIntegratedOrder']['name_applicant'] = $req_data['UserIntegratedOrder']['name_applicant'];
				$data['UserIntegratedOrder']['phone'] = $req_data['UserIntegratedOrder']['phone'];
				$data['UserIntegratedOrder']['addon_r123'] = $addon_r123;
				$data['UserIntegratedOrder']['addon_olx'] = $addon_olx;
				$data['UserIntegratedOrder']['region_id'] = $req_data['UserIntegratedOrder']['region_id'];
				$data['UserIntegratedOrder']['city_id'] = $req_data['UserIntegratedOrder']['city_id'];
				$data['UserIntegratedOrder']['subarea_id'] = $req_data['UserIntegratedOrder']['subarea_id'];
				$data['UserIntegratedOrder']['address'] = $req_data['UserIntegratedOrder']['address'];
				$data['UserIntegratedOrder']['zip'] = $req_data['UserIntegratedOrder']['zip'];

				$data['UserIntegratedOrderAddon']['r123_package_id'] = $r123_package_id;
				$data['UserIntegratedOrderAddon']['olx_package_id'] = $olx_package_id;
				
				if ($is_all_addon) {
					$data['UserIntegratedOrder']['is_email_all_addon'] = true;
					$data['UserIntegratedOrder']['email_all_addon'] = $req_data['UserIntegratedOrder']['email_all_addon'];
					$data['UserIntegratedOrder']['email_r123'] = $req_data['UserIntegratedOrder']['email_all_addon'];
					$data['UserIntegratedOrder']['email_olx'] = $req_data['UserIntegratedOrder']['email_all_addon'];
				}
			}
			// debug($data);
			// debug($req_data);die();
