  	...
  	<tbody>
	  	<tr>
	  		<td style="padding: 20px 0;">
	      		<?php
		      			echo $this->Html->tag('h1', sprintf(__('Referral KPR %s telah disetujui'), $code), array(
	      		...
	      		<?php
						if( !empty($mls_id) ) {
			      			echo $this->Html->tag('h4', __('Informasi Pengajuan sebagai berikut:'), array(
								'style' => 'margin-top:-10px; font-weight:400; padding-bottom: 10px;font-size: 16px;margin: 10px 0 0;'
							));
	  	...
	  	<tr>
	    	<td>
	      		<?php
						echo $this->Html->tag('p', __('Mohon maaf, Anda tidak mendapatkan Provisi untuk Pengajuan KPR ini.'), array(
							'style' => 'margin:20px 0;font-size: 18px;font-weight:400;',
						));
				?>
	    	</td>
	  	</tr>
