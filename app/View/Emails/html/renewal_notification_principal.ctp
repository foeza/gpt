		...
		<tr>
			<td>
				...
				//	remove extra slash
					...
					$detailURL = $this->Html->link(__('Perpanjang Masa Tayang'), $detailURL, array(
						'style'		=> 'display: block; color: #fff; text-align: center; text-decoration: none; margin: 0 20px;', 
						'target'	=> '_blank', 
					));

					echo($this->Html->tag('p', __('Perpanjang masa tayang website Anda dengan menekan tombol di bawah ini :'), array(
		...
		<tr>
			<td align="center">
				<?php
					...
					$membershipURL = $this->Html->link(__('di sini'), $membershipURL, array(
						'target' => '_blank', 
					));

					echo($this->Html->tag('p', __('atau, Anda dapat memilih paket membership lainnya %s', $membershipURL), array(
						'style'	=> 'padding: 20px 0; margin: 0; color: #8b8b8b; line-height: 24px;', 
					)));
