<?php 
		...
		echo $this->Html->tag('p', sprintf(__('Properti Anda tidak terupdate/refresh selama %s hari, dalam 7 hari kedepan properti Anda akan Kami non-aktifkan.'), $expired_day), array(
			'style' => 'margin: 15px 0 5px;'
		));
		echo $this->Html->tag('p', __('Mohon update/refresh kembali apabila iklan tersebut masih available: '), array(
			...
			<td>
				<?php 
						echo $this->element('emails/html/properties/info', array(
							'params' => $value,
							'url' => $this->Html->link(__('Lihat'), $url, array(
								'escape' => false,
