<?php 
		...
		if($status == 'approve'){
			$header = $this->Html->tag('p', sprintf(__('Selamat! pengajuan aplikasi Co-Broke Anda dengan kode Co-Broke #%s dan judul properti "%s" telah disetujui.'), $code, $title), array(
			...
			$header = $this->Html->tag('p', sprintf(__('Maaf, pengajuan aplikasi Co-Broke Anda dengan kode Co-Broke #%s dan judul properti "%s" tidak disetujui oleh agen yang bersangkutan dengan alasan : <b>%s</b>'), $code, $title, $decline_reason), array(
					'style' => 'color: #303030; font-size: 14px; margin: 5px 0 20px; line-height: 20px;'
				));
			$content = $this->Html->tag('p', __('Harap hubungi agen atau perusahaan terkait jika ingin melakukan kerja sama ini lebih lanjut, terima kasih.'), array(
					'style' => 'color: #303030; font-size: 14px; margin: 5px 0 20px; line-height: 20px;'
				));
		}
		...
		<td style="width: 50%;padding-left: 10px;">
			<?php
					...
					if(!empty($final_commission)){
						echo $this->Html->tag('div', sprintf('Komisi: <b>%s</b>', $commission_text), array(
							'class' => 'bt-comission',
