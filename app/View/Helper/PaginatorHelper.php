 ...
 * @var array
 */
	public $helpers = array('Html', 'Rumahku');
 ...
 */
	public function sort($key, $title = null, $options = array()) {
		...
		$defaultDir = $direction = isset($options['direction']) ? $options['direction'] : 'asc';

		// custom dari doni yang diambil bukan dari paginate
		$sort = !empty($options['sort']) ? $options['sort'] : false; 
		$customize = !empty($options['customize']) ? $options['customize'] : false; 
		...
		if($customize && $key && $direction){
			if($key == $sort){
				$options['class'] .= ' '.$direction;
			}
		}

		$url = array_merge(array('sort' => $key, 'direction' => $dir), $url, array('order' => null));

		return $this->link($title.$this->Rumahku->icon('rv4-sort-1'), $url, $options);
	}
 ...
 */
	protected function _pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		$_defaults = array(
			...
			'only_url' => isset($options['only_url']) ? $options['only_url'] : false
		);
		...
		foreach (array_keys($_defaults) as $key) {
			...
			if($key != 'only_url'){
				unset($options[$key]);
			}else if($key == 'only_url' && empty($options[$key])){
				unset($options[$key]);
			}
		}

		if( !empty($options['class-link']) ) {
			$options['class'] = $options['class-link'];
			unset($options['class-link']);
		}
 ...
 */
	public function counter($options = array()) {
		...
		$start = 0;
		$paging['count'] = isset($paging['count'])?$paging['count']:0;
		$paging['limit'] = isset($paging['limit'])?$paging['limit']:0;
		$paging['current'] = isset($paging['current'])?$paging['current']:false;
 ...
 */
	public function numbers($options = array()) {
		$havePage = Configure::write('Pagination.HavePage');
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			for ($i = $start; $i < $params['page']; $i++) {
				if( $i == 1 && !empty($currentTag) ) {
					$options['page_first'] = true;
				}
			...
			if ($currentTag) {
				...
				if(!$only_url){
					$out .= $this->Html->tag($tag, 
						$this->link($params['page'], array(
							'page' => $params['page']
			...
			if ($i != $params['pageCount'] && !$only_url) {
				$out .= $separator;
			}
			...
			if ($last && $end < $params['pageCount'] && !$only_url) {
				$offset = ($params['pageCount'] < $end + (int)$last) ? $params['pageCount'] - $end : $last;
				if ($offset <= $last && $params['pageCount'] - $end > $offset) {
					$out .= $this->last($offset, compact('tag', 'separator', 'ellipsis', 'class', 'url'));
				} else {
					$out .= $this->last($offset, compact('tag', 'separator', 'class', 'ellipsis', 'url') + array('before' => $separator));
				}
			}
 ...
 */
	public function last($last = 'last >>', $options = array()) {
		...
		if (is_int($last) && $params['page'] <= $lower) {
			...
			if(!empty($only_url)){
				$out = array('page' => $params['pageCount']);
			}else{
