...
Class RestComponent extends Component {
	...
	public function beforeRender (Controller $Controller) {
		...
		/* INI HASIL CUSTOM */
		$var_data = $this->Controller->viewVars;
		$setting_api = Configure::read('__Site.settings');
		...
		if(!empty($setting_api['Setting']['is_mobile_api'])){
			$not_to_custom = array(
			...
			if(!empty($data)){
				foreach ($data as $key => $value) {
					if(!in_array($key, $not_to_custom) && is_array($value)){
						$data[$key] = $this->convert_array($value);
					}
				}
			}
		}
	 ...
	 * @return boolean
	 */
	public function logs ($key, $val = null) {
		// Write log
	...
	/*INI BIKIN SENDIRI*/
	public function convert_array($data, $empty = null){
		if(!empty($data)){
			foreach ($data as $key => $value) {
				if(!is_numeric($key) && is_array($value)){
					foreach ($value as $field => $val_field) {
	                    if( is_bool($val_field) ){
	                        $data[$key][$field] = ($val_field === true) ? 1 : $empty;
	                    }else if(!empty($val_field) && is_array($val_field)){
	                    	$data[$key][$field] = $this->convert_array($val_field);
	                    }
	                }
				}else if(is_array($value)){
					foreach ($value as $model => $real_value) {
			            if(!empty($real_value) && is_array($real_value)){
			                foreach ($real_value as $field => $val_field) {
			                    if(is_bool($val_field)){
			                        $data[$key][$model][$field] = ($val_field === true) ? 1 : $empty;
			                    }else if(!empty($val_field) && is_array($val_field)){
			                    	$temp[$field] = $val_field;
			                    	$temp = $this->convert_array($temp);
			                    	
			                    	$data[$key][$model][$field] = $temp[$field];
			                    }
			                }
			            }
					}
