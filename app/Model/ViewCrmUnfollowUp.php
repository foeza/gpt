<?php
class ViewCrmUnfollowUp extends AppModel {
	var $name = 'ViewCrmUnfollowUp';
	...
	function getData( $find = 'all', $options = array(), $elements = array() ){
		...
		$default_options = array(
			'conditions' => array(),
			'order' => array(
				'ViewCrmUnfollowUp.activity_datetime' => 'DESC',
				'ViewCrmUnfollowUp.id' => 'DESC',
			),
			'contain' => array(),
            'fields' => array(),
            'group' => array(),
        ...
        if( !empty($mine) ) {
            ...
            if( (empty($user_admin) && empty($adminRumahku)) || !empty($is_sales) ) {
                $default_options['conditions']['ViewCrmUnfollowUp.user_id'] = $user_ids;
                $company = false;
            } else {
            	$company = true;
            }
        ...
        if( !empty($company) ) {
            ...
            $default_options['conditions']['ViewCrmUnfollowUp.user_id'] = $agent_id;
        }
