<?php
class PropertyFilterDetail extends AppModel {
//	untuk clear cache
	var $name		= 'PropertyFilterDetail';
	var $belongsTo	= array(
	...
	public function afterSave($created, $options = array()){
		...
		if(isset($this->id) && $this->id){
			$filterID = $this->fields($this->alias.'.property_filter_id', array(
				$this->alias.'.id' => $this->id, 
			));

			$cacheConfig	= 'properties_filter';
			$cacheName		= sprintf('Properties.Filter.%s.%s', $companyID, $filterID);
	...
	public function getMerge($records = array()){
		if($records){
			$isMultiple	= Hash::numeric(array_keys($records));
			$records	= empty($isMultiple) ? array($records) : $records;

			foreach($records as $key => $result){
				$recordID		= Common::hashEmptyField($result, 'PropertyFilter.id');
				$filterDetails	= $this->getData('all', array(
					'conditions' => array(
						$this->alias.'.property_filter_id' => $recordID, 
					), 
				));

				$records[$key] = array_merge($result, array(
					$this->alias => $filterDetails, 
				));
			}

		//	convert back to single format (kalo pertamanya single format)
			$records = empty($isMultiple) ? array_shift($records) : $records;
		}
