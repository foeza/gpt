...
google.loader.rfm({":search":{"versions":{":1":"1",":1.0":"1"},"path":"/api/search/1.0/56f70d816baa48bdfe9284ebc883ad41/","js":"default+id.I.js","css":"default+id.css","properties":{":Version":"1.0",":NoOldNames":false,":JSHash":"56f70d816baa48bdfe9284ebc883ad41"}},":language":{"versions":{":1":"1",":1.0":"1"},"path":"/api/language/1.0/cddd1e0315f352539fc3732010bda57f/","js":"default+id.I.js","properties":{":Version":"1.0",":JSHash":"cddd1e0315f352539fc3732010bda57f"}},":annotations":{"versions":{":1":"1",":1.0":"1"},"path":"/api/annotations/1.0/69e4c522bede40545878d7b96b100995/","js":"default+id.I.js","properties":{":Version":"1.0",":JSHash":"69e4c522bede40545878d7b96b100995"}},":wave":{"versions":{":1":"1",":1.0":"1"},"path":"/api/wave/1.0/3b6f7573ff78da6602dda5e09c9025bf/","js":"default.I.js","properties":{":Version":"1.0",":JSHash":"3b6f7573ff78da6602dda5e09c9025bf"}},":earth":{"versions":{":1":"1",":1.0":"1"},"path":"/api/earth/1.0/db22e5693e0a8de1f62f3536f5a8d7d3/","js":"default.I.js","properties":{":Version":"1.0",":JSHash":"db22e5693e0a8de1f62f3536f5a8d7d3"}},":feeds":{"versions":{":1":"1",":1.0":"1"},"path":"/api/feeds/1.0/482f2817cdf8982edf2e5669f9e3a627/","js":"default+id.I.js","css":"default+id.css","properties":{":Version":"1.0",":JSHash":"482f2817cdf8982edf2e5669f9e3a627"}},":picker":{"versions":{":1":"1",":1.0":"1"},"path":"/api/picker/1.0/1c635e91b9d0c082c660a42091913907/","js":"default.I.js","css":"default.css","properties":{":Version":"1.0",":JSHash":"1c635e91b9d0c082c660a42091913907"}},":ima":{"versions":{":3":"1",":3.0":"1"},"path":"/api/ima/3.0/28a914332232c9a8ac0ae8da68b1006e/","js":"default.I.js","properties":{":Version":"3.0",":JSHash":"28a914332232c9a8ac0ae8da68b1006e"}}});
...
(function ( $ ) {
    ...
    $.gChart = function(options, selector_id){
        var settings = $.extend({
            obj: 'chart_div',
            loadCallback: true,
        ...
        var settingsPie = $.extend({
            obj: 'pie_chart_div',
            loadCallback: true,
            value: $('#pieChartValue').val(),
        }, options );

        var selector_id = ( typeof selector_id != 'undefined') ? selector_id : 'wrapper-chart-kpr';
        ...
        if(typeof google != 'undefined' ) {
            ...
            function drawBasic() {
            	if( typeof settings.value != 'undefined' ) {
	                ...
	                if( $('.admin-report').length ) {
	                	colors = ['#F6B60E'];
	                }

	                if( value != '' ) {
		                value = $.parseJSON(value);
		                flagZero = Array.apply(null, Array(value[0].length)).map(function (x, i) { return null; });
		                for(var x in value){
		                	// additional loop to check whether all values is = 0 or not, if all = 0 then hide all dot
	                ...
	                data.addColumn('string', 'Bulan'); // Implicit domain label col.
	                if( $('.kpr-chart.active').attr('chart-for') == 'properties' ) {
	                	colors = ['#069E55', '#FF9600', '#F3289E'];
	                	data.addColumn('number', 'Properti Terjual');
	                	data.addColumn('number', 'Properti Tersewa');
	                	data.addColumn('number', 'Pertambahan Properti');
	                } else {
	                	var point_tooltip = 'User';
	                	if( $('.wrapper-selector').attr('chart-point-tooltip') != 'undefined' ) {
	                		point_tooltip = $('.wrapper-selector').attr('chart-point-tooltip');
	                	}
	                	data.addColumn('number', point_tooltip); // Implicit series 1 data col.
	                }
	                ...
	                var chart = new google.visualization.LineChart(document.getElementById(settings.obj));
	                chart.draw(data, options);
            	}    		
            }

            function drawPieBasic() {
            	if( typeof settingsPie.value != 'undefined' ) {
            		var data = new google.visualization.DataTable();
	                var value = settingsPie.value;
	                ...
	                data.addColumn('string', 'Kota'); // Implicit domain label col.
	                data.addColumn('number', 'total'); // Implicit domain label col.
	                data.addRows(arr);
	                var options = {
	                ...
	                var chart = new google.visualization.PieChart(document.getElementById(settingsPie.obj));
	        		chart.draw(data, options);
            	}
            }
            ...
            if( is_first_load ) {
            	is_first_load = false;
            	if( settings.loadCallback == true ) {
	            	google.setOnLoadCallback(drawBasic);
	            }
            	if( settingsPie.loadCallback == true ) {
	            	google.setOnLoadCallback(drawPieBasic);
	            }
            } else {
            	// if( selector_id == 'wrapper-chart' ) {
	            	if( settings.loadCallback == true ) {
		            	google.setOnLoadCallback(drawBasic);
		            } else {
		            	drawBasic();
	            ...
	            // else if( selector_id == 'wrapper-pie-chart' ) {
    ...
    $.gChartDateRange = function( startChangeDate, endChangeDate ){
    	var flagFormat = false;
    	var noFormat = false;
    	var range_options = false;

    	var _initialize = function ( startDate, endDate ) {
    		if( $('#DateFromPicker').length > 0 ) {
	        	startChangeDate = $('#DateFromPicker').val();
	            noFormat = true;
	    	} else if(typeof startDate == 'undefined' ) {
	            startChangeDate = moment().subtract('days', 30).format('DD/MM/YYYY');
	            flagFormat = true;
	        } else {
	        	startChangeDate = $.getDates(startDate, true);
	        }

	    	if( $('#DateToPicker').length > 0 ) {
	        	endChangeDate = $('#DateToPicker').val();
	        } else if(typeof endDate == 'undefined' ) {
	            endChangeDate = moment().format('DD/MM/YYYY');
	        } else {
	        	endChangeDate = $.getDates(endDate, true);
	        }
    	...
    	_initialize( startChangeDate, endChangeDate );

	 //    if( noFormat == false ) {
		//     if( flagFormat == true ) {
		//     	startChangeDate = startChangeDate.format('YYYY-MM-DD');
		//     	endChangeDate= endChangeDate.format('YYYY-MM-DD');
		//     } else {
		// 	    startChangeDate = startChangeDate.getFullYear()+'-'+(startChangeDate.getMonth()+1)+'-'+startChangeDate.getDate();
		// 	    endChangeDate= endChangeDate.getFullYear()+'-'+(endChangeDate.getMonth()+1)+'-'+endChangeDate.getDate();
		// 	}
		// }

	  // 	var fillContent = function ( result ) {
	  //   	var content = $(result).filter('#wrapper-chart-kpr').html();
   //          var value = $(result).find('#chartValue').val();

   //          $('#wrapper-chart-kpr').html(content);
   //          $.gChart({
   //              loadCallback: false,
   //              value: value,
   //          });
			// $.gChartDateRange(startChangeDate, endChangeDate);
	  //   }
	 	
	 	// UNDER DEVELOPMENT
	 	var fillContent = function ( result, selector_id ) {
	    	if( $(selector_id).length > 0 ) {
    			var content = $(result).filter(selector_id).html();

                if(typeof content == 'undefined' ) {
	    			content = $(result).find(selector_id).html();
                }
	    		
	    		var value = $(result).find('#chartValue').val();

	    		$(selector_id).html(content);
	    		$.gChart({
	                loadCallback: false,
	                value: value,
	            }, selector_id.substr(1));
	    	}

    		_initialize( startChangeDate, endChangeDate );
	   //  	range_options['startDate'] = startChangeDate;
	   //  	range_options['endDate'] = endChangeDate;
            
            if( selector_id == '#wrapper-chart' || selector_id == '#wrapper-chart-kpr' ) {
				dateRangeDasboard(range_options);
            } else if( selector_id == '#wrapper-dashboard-table-property-active' ) {
				dateRangeDasboardPropertyActive(range_options);
            } else if( selector_id == '#wrapper-dashboard-table-property-sold' ) {
				dateRangeDasboardPropertySold(range_options);
            } else if( selector_id == '#wrapper-dashboard-table-ebrosur' ) {
				dateRangeDasboardEbrosur(range_options);
            }
			// $.gChartDateRange(startChangeDate, endChangeDate);
			kprChart();
	    }

	    function execAjax( self, start, end, selector_id ) {
	    	
	    	var ajax_url = self.attr('url');

	    	if( start && end ) {
	    		startChangeDate = start.format('YYYY-MM-DD');
		        endChangeDate = end.format('YYYY-MM-DD');

		        ajax_url += '/'+startChangeDate+'/'+endChangeDate+'/';
	    	}

	        $.ajax({
	            url: ajax_url,
	            success: function(result) {
	                fillContent(result, selector_id);
	            },
	    ...
	    var dateRangeReport = function () {
		    $('.daterange-report').daterangepicker(range_options);
		    $('.daterange-report').on('apply.daterangepicker', function(ev, picker) {
		        $('#SearchDate').val( picker.startDate.format('DD/MM/YYYY') + ' - ' + picker.endDate.format('DD/MM/YYYY')).parents('form').submit();
	    ...
	    var dateRangeDasboard = function (range_options) {
		    $('.daterange-dasboard').daterangepicker(range_options);
		    $('.daterange-dasboard').on('apply.daterangepicker', function(ev, picker) {
		    	var start = picker.startDate;
		    	var end = picker.endDate;
		        var self = $('.daterange-dasboard');
		        var selector_id = '#wrapper-chart';

		        if( self.parents('.wrapper-selector').length ) {
		        	selector_id = '#' + self.parents('.wrapper-selector').attr('id');
		        	if( self.parents('.wrapper-selector').attr('data-id') !== undefined ) {
			        	selector_id = '#' + self.parents('.wrapper-selector').attr('data-id');
			        }
		        }

		        execAjax(self, start, end, selector_id);
	    ...
	    var dateRangeDasboardPropertyActive = function (range_options) {
		    $('.daterange-dasboard-table-property-active').daterangepicker(range_options);
		    $('.daterange-dasboard-table-property-active').on('apply.daterangepicker', function(ev, picker) {
		    	var start = picker.startDate;
		    	var end = picker.endDate;
		        
		        var self = $('.daterange-dasboard-table-property-active');
		        execAjax(self, start, end, '#wrapper-dashboard-table-property-active');
	    ...
	    var dateRangeDasboardPropertySold = function (range_options) {
		    $('.daterange-dasboard-table-property-sold').daterangepicker(range_options);
		    $('.daterange-dasboard-table-property-sold').on('apply.daterangepicker', function(ev, picker) {
		    	var start = picker.startDate;
		    	var end = picker.endDate;
		        
		        var self = $('.daterange-dasboard-table-property-sold');
		        execAjax(self, start, end, '#wrapper-dashboard-table-property-sold');
	    ...
	    var dateRangeDasboardEbrosur = function (range_options) {
		    $('.daterange-dasboard-table-ebrosur').daterangepicker(range_options);
		    $('.daterange-dasboard-table-ebrosur').on('apply.daterangepicker', function(ev, picker) {
		    	var start = picker.startDate;
		    	var end = picker.endDate;
		        
		        var self = $('.daterange-dasboard-table-ebrosur');
		        execAjax(self, start, end, '#wrapper-dashboard-table-ebrosur');
	    ...
	    var kprChart = function () {
		    if( $('.kpr-chart').length ) {
        		$('.kpr-chart').off('click');
		    	$('.kpr-chart').click(function() {
			        var self = $(this);
			        var ajax_url = self.attr('url');
			        var id = '#'+self.closest('.wrapper-selector').attr('id');

			        if( id == '#wrapper-chart-commission' ) {
			        	var year = $('#filter_commissionYear').val();
			        	var month = $('#filter_commission_month').val();
			        	...
			        	ajax_url += '/'+start_date+'/'+end_date+'/';
			        } else if( startChangeDate != false && endChangeDate != false ) {
		        		startChangeDate = $.getDates(startChangeDate);
		        		endChangeDate = $.getDates(endChangeDate);
			        ...
			        $.ajax({
			            url: ajax_url,
			            success: function(result) {
				            // fillContent(result, '#wrapper-chart-kpr');
				            fillContent(result, id);
			            },
			            error: function(XMLHttpRequest, textStatus, errorThrown) {
			                alert('Error load Data');
			            }
        ...
        // PIE CHART FILTER
        // UNDER DEVELOPMENT
	    // if( $('#pie_chart_filter').length ) {

	    // 	$('#pie_chart_filter').unbind('change');
	    // 	$('#pie_chart_filter').change(function(){
	    // 		var self = $(this);
	    // 		var value = self.val();
	    //     	var ajax_url = self.attr('url') + '/' + value;
	    //     	self.attr('url', ajax_url)	
	    //     	execAjax(self, false, false, '#wrapper-pie-chart');
	    // 	});
	    // }
	};
