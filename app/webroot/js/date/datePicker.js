        ...
        Template = $('<div class="rangePicker"> \
        				<div class="wrap"> \
							...
							<div class="custom"> \
								...
								<footer> \
									<button type="button" class="confirm btn btn-bg green">Apply</button> \
		...
		RangePicker.prototype = {
			...
			//	this.settings.presets.push({ buttonText:'custom', value:'custom' });
				...
				this.calendar.from.append(
					$('<div>').addClass('form-group select-group-wrapper').html($('<select>').html(years)),  // years.replace(/%%/g,'From - ')
			...
			monthClick : function(e){
				...
				this.changeMonth( calendarIdx, monthIdx, 'click' );
			},
			...
			changeMonth : function(calendarIdx, monthIdx, trigger){
				var that = this,
				...
				var arrStr = '[['+this.result[0][0]+','+this.result[0][1]+'],['+this.result[1][0]+','+this.result[1][1]+']]';
				this.obj.attr('data-value', arrStr);

				if( $('.select-periode-picker').length > 0 && trigger == 'click' ) {
					var month_value = parseInt($('.select-periode-picker').val());
					var change_index = calendarIdx;

					if( calendarIdx == 0 ) {
						var opposite_month = monthIdx + (month_value-1);

						change_index = 1;

						if( opposite_month >= 12 ) {
							var current_year = parseInt(this.result[calendarIdx][1]) + 1;
							opposite_month = opposite_month - 12;

							this.result[change_index][1] = current_year;
							
							this.picker.find('.calendar.to').find('select').val(current_year).trigger('change');
						} else if( this.result[0][1] != this.result[1][1] ) {
							this.picker.find('.calendar.to').find('select').val(this.result[0][1]).trigger('change');
						}
						...
						if( opposite_month < 0 ) {
							...
							opposite_month = Math.abs(opposite_month);
							opposite_month = opposite_month - 12;
							opposite_month = Math.abs(opposite_month);

							this.result[change_index][1] = current_year;
							
							this.picker.find('.calendar.from').find('select').val(current_year).trigger('change');
						} else if( this.result[0][1] != this.result[1][1] ) {
							this.picker.find('.calendar.from').find('select').val(this.result[1][1]).trigger('change');
						}
					}

					that.changeMonth( change_index, opposite_month );
					this.summary();
				}
